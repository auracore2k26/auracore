...
class Store {
  ...
  impersonateUser(role, targetId) {
    if (role === 'trainer') {
      ...
      if (t) {
        this.data.currentUser = { role: 'trainer', id: t.id, name: t.name };
        this.saveState();
      ...
      if (c) {
        this.data.currentUser = { role: 'client', id: c.id, name: c.name };
        this.saveState();
    ...
    <div style="background: linear-gradient(135deg, rgba(0, 229, 255, 0.15), rgba(124, 77, 255, 0.2)); border: 1px solid var(--accent-cyan); border-radius: var(--radius-md); padding: 1.25rem 1.5rem; margin-bottom: 1.5rem; box-shadow: 0 0 25px rgba(0, 229, 255, 0.2);">
      <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; margin-bottom: 1rem;">
        <div>
          ...
          <p style="font-size: 0.85rem; color: var(--text-muted);">Navigate dedicated Admin pages, edit credentials, or perform instant role impersonations.</p>
        </div>
        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
          <button class="btn btn-secondary btn-sm" id="btn-change-admin-pass" style="font-size: 0.85rem;">
            🔑 Change Password
          </button>
      ...
      <div class="modal-container" style="max-width: 580px;">
        ...
        <div style="background: rgba(255,255,255,0.03); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 1rem; margin-bottom: 1rem;">
          <h4 style="font-size: 0.85rem; color: var(--accent-cyan); margin-bottom: 0.5rem; font-weight: 700;">
            📝 Already Paid? Submit 12-Digit Transaction Reference (UTR)
          </h4>
          <form id="form-submit-utr" style="display: flex; gap: 0.5rem;">
            <input type="text" id="utr-number" class="form-control" placeholder="Enter 12-digit UTR / Ref No." required style="font-size: 0.85rem;" />
      ...
      <div class="modal-container">
        ...
        <form id="trainer-form">
          ...
          <div class="form-group">
            ...
            <select id="trn-specialty" class="form-control">
              <option value="Personal Fitness & Body Transformation" ${isEdit && existingTrainer.specialty === 'Personal Fitness & Body Transformation' ? 'selected' : ''}>Personal Fitness & Body Transformation</option>
              <option value="Weight Loss & Functional Fitness" ${isEdit && existingTrainer.specialty === 'Weight Loss & Functional Fitness' ? 'selected' : ''}>Weight Loss & Functional Fitness</option>
              <option value="Strength & Conditioning" ${isEdit && existingTrainer.specialty === 'Strength & Conditioning' ? 'selected' : ''}>Strength & Conditioning</option>
              <option value="Special Care & Mobility Training" ${isEdit && existingTrainer.specialty === 'Special Care & Mobility Training' ? 'selected' : ''}>Special Care & Mobility Training</option>
            </select>
          </div>
          <div class="form-group">
    ...
    <div class="modal-overlay active" id="client-modal">
      <div class="modal-container">
        ...
        <form id="client-form">
          ...
          <div class="form-group">
            <label class="form-label">Phone Number</label>
            <input type="text" id="cli-phone" class="form-control" value="${isEdit ? existingClient.phone : ''}" required />
          </div>
          <div class="form-group">
...
function renderTrainerView(container) {
            ...
            return `
              <div style="background: rgba(255,255,255,0.03); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 1.25rem;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem;">
                  <div>
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                      ...
                      <button class="btn btn-secondary btn-sm edit-goal-btn" data-client-id="${client.id}" style="font-size: 0.7rem; padding: 0.1rem 0.4rem;">✍️ Edit Goal</button>
                    </div>
                  </div>
                  <span class="badge ${isOffline ? 'badge-emerald' : 'badge-purple'}">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; margin-bottom: 0.5rem;">
                  ${isOffline ? `
                    <button class="btn btn-secondary btn-pin-location" data-client-id="${client.id}" style="font-size: 0.75rem;">
                      📍 Pin GPS
                    </button>
                  ` : `
                    <a href="${client.onlineLink || 'https://meet.google.com/aur-acor-fit'}" target="_blank" class="btn btn-secondary" style="text-decoration: none; font-size: 0.75rem;">
                      📹 Google Meet
                    </a>
                  `}

                  <button class="btn btn-secondary qr-class-join-btn" data-client-name="${client.name}" data-client-id="${client.id}" style="font-size: 0.75rem;">
                    📱 Join QR Code
                  </button>

                  <button class="btn ${isSessionActive ? 'btn-emerald' : 'btn-secondary'} btn-start-class" data-client-id="${client.id}" ${isSessionActive ? 'disabled' : ''} style="font-size: 0.75rem;">
                    ${isSessionActive ? '⏳ Class Pending' : '▶️ Verification'}
                  </button>
                </div>

                <button class="btn btn-emerald btn-direct-attendance" data-client-id="${client.id}" data-client-name="${client.name}" style="width: 100%; font-size: 0.85rem;">
                  ✅ Mark Attendance Directly (+1 Class)
                </button>
              </div>
            `;
          }).join('')}
  ...
  container.querySelectorAll('.btn-direct-attendance').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const cliId = e.currentTarget.getAttribute('data-client-id');
      const cliName = e.currentTarget.getAttribute('data-client-name');
      if (confirm(`Manually mark 1 completed class for ${cliName}?`)) {
        const res = store.markAttendanceDirectly(cliId, currentTrainer.id);
        if (res.success) {
          alert(`🎉 Attendance marked successfully for ${cliName}!\nProgress: ${res.client.currentCycleClasses} / ${res.client.totalCycleClasses || 12} Classes Completed.`);
        }
      }
    });
  });
  ...
  container.querySelectorAll('.edit-goal-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const cliId = e.currentTarget.getAttribute('data-client-id');
      const cli = data.clients.find(c => c.id === cliId);
      if (cli) openEditGoalModal(cli);
    });
  });

  container.querySelectorAll('.qr-class-join-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const name = e.currentTarget.getAttribute('data-client-name');
      const id = e.currentTarget.getAttribute('data-client-id');
      openClassJoinQRModal(`Class Join QR for ${name}`, `https://auracore.app/join-session?studentId=${id}&trainerId=${currentTrainer.id}`);
    });
  });
...
function renderClientView(container) {
  ...
  container.innerHTML = `
    ...
    <div class="grid-2" style="margin-bottom: 2rem;">
      ...
      <div class="card">
        ...
        <button class="btn btn-secondary" id="btn-student-qr-join" style="width: 100%; margin-bottom: 0.75rem;">
          📱 Show Join / Attendance QR Code
        </button>
  ...
  const qrJoinBtn = container.querySelector('#btn-student-qr-join');
  if (qrJoinBtn) {
    qrJoinBtn.addEventListener('click', () => {
      openClassJoinQRModal(`Student Attendance & Join QR (${currentClient.name})`, `https://auracore.app/join-student?id=${currentClient.id}`);
    });
  }
