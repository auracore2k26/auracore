...
const DEFAULT_SEED_DATA = {
  ...
  paymentSettings: {
    upiId: '7550249618@upi',
    helplinePhone: '75502 49618',
...
async function syncToSupabaseCloud(data) {
  try {
    ...
    if (data.paymentSettings) {
      const dbPay = [{
        id: 1,
        upi_id: data.paymentSettings.upiId || '7550249618@upi',
        helpline_phone: data.paymentSettings.helplinePhone || '75502 49618',
  ...
  try {
    ...
    if (payRes && payRes.length > 0) {
      ...
      store.data.paymentSettings = {
        upiId: p.upi_id || '7550249618@upi',
        helplinePhone: p.helpline_phone || '75502 49618',
        companyName: 'AuraCore Fitness & Care',
...
class Store {
  ...
  loadState() {
    try {
      ...
      if (saved) {
        ...
        if (!parsed.paymentSettings || (parsed.paymentSettings.helplinePhone && parsed.paymentSettings.helplinePhone.includes('87547'))) {
          parsed.paymentSettings = { ...DEFAULT_SEED_DATA.paymentSettings };
        } else {
          parsed.paymentSettings.helplinePhone = '75502 49618';
          if (parsed.paymentSettings.upiId && parsed.paymentSettings.upiId.includes('87547')) {
            parsed.paymentSettings.upiId = '7550249618@upi';
          }
        }
        return parsed;
      }
  ...
  updatePaymentSettings(upiId, helplinePhone, companyName, defaultFeeAmount, customQrImage) {
    this.data.paymentSettings = {
      upiId: upiId || '7550249618@upi',
      helplinePhone: helplinePhone || '75502 49618',
  ...
  confirmClassSession(sessionId) {
    if (!this.data.activeVerificationSession) {
      return false;
    }
    
    const sess = this.data.activeVerificationSession;
    if (sessionId && sess.id !== sessionId) {
      console.warn('Session ID mismatch, but resolving active verification session:', sess.id, 'vs', sessionId);
    }
...
function getCurrentLocation() {
  return new Promise((resolve) => {
    ...
    // Guaranteed instant fallback timer (resolves within 800ms no matter what)
...
function getActiveUPIID() {
  return (store.data.paymentSettings && store.data.paymentSettings.upiId) || '7550249618@upi';
}

function getActiveHelpline() {
  return (store.data.paymentSettings && store.data.paymentSettings.helplinePhone) || '75502 49618';
}
      ...
      <div class="modal-container">
        ...
        <form id="payment-settings-form">
          <div class="form-group">
            ...
            <input type="text" id="set-upi" class="form-control" value="${settings.upiId || '7550249618@upi'}" required />
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" id="set-phone" class="form-control" value="${settings.helplinePhone || '75502 49618'}" required />
          </div>
...
function renderTrainerView(container) {
  ...
  container.querySelectorAll('.btn-pin-location').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      const clientId = targetBtn.getAttribute('data-client-id');
      const client = data.clients.find(c => c.id === clientId);
      
      targetBtn.innerText = '🛰️ GPS...';
  ...
  container.querySelectorAll('.btn-start-class').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      const clientId = targetBtn.getAttribute('data-client-id');
      if (!clientId) {
        alert('Could not find student ID for session start.');
        return;
      }
...
function renderClientView(container) {
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener('click', (e) => {
      const targetBtn = e.currentTarget;
      const sessId = targetBtn.getAttribute('data-session-id') || (store.data.activeVerificationSession ? store.data.activeVerificationSession.id : null);
      const success = store.confirmClassSession(sessId);
      if (success) {
        alert('🎉 Attendance Confirmed!\n+1 Class added to your batch progress and verified into AuraCore system.');
      } else {
        alert('Unable to confirm class. Please ask your Trainer to click "▶️ Verification Session" again.');
      }
...
function renderWhatsAppSupportWidget() {
  ...
  const helpline = (store.data.paymentSettings && store.data.paymentSettings.helplinePhone) || '75502 49618';
  const digits = helpline.replace(/\D/g, '');
  const cleanPhone = digits.length === 10 ? '91' + digits : (digits || '917550249618');
  ...
  const widgetHTML = `
    <a href="${waUrl}" target="_blank" class="whatsapp-float-btn" id="aura-whatsapp-widget" title="Chat on WhatsApp (75502 49618)">
...
function syncHeaderHelplinePhone() {
  const phone = (store.data.paymentSettings && store.data.paymentSettings.helplinePhone) || '75502 49618';
  const cleanDigits = phone.replace(/\D/g, '');
  const waDigits = cleanDigits.length === 10 ? '91' + cleanDigits : (cleanDigits || '917550249618');
